module Invidious::Routes::API::V1::Authenticated
  ...
  def self.feed(env)
    ...
    videos, notifications, shorts, _shorts_available, _total_fetched = get_subscription_feed(user, max_results, page, false)
    ...
    JSON.build do |json|
      json.object do
        ...
        json.field "shorts" do
          json.array do
            shorts.each do |video|
              video.to_json(locale, json)
            end
          end
        end
