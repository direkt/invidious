  ...
  def _post_invidious_companion(
    ...
    begin
      ...
      COMPANION_POOL.client do |wrapper|
        ...
        full_url = "#{companion_base_url}#{endpoint}"
        LOGGER.debug("Companion request: POST #{full_url}")

        wrapper.client.post(full_url, headers: headers, body: data.to_json) do |response|
          if response.status_code != 200
            error_body = response.body_io.gets_to_end
            LOGGER.error("Companion error (#{response.status_code}): #{error_body}")
            raise InfoException.new("Companion returned #{response.status_code}: #{error_body[0..100]}")
          end
