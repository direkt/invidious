...
module Invidious::Routes::Feeds
  ...
  def self.subscriptions(env)
    ...
    videos, notifications, shorts = get_subscription_feed(user, max_results, page)

    # Show the Shorts tab if the user hides shorts OR if there are shorts available
    show_shorts_tab = user.preferences.hide_shorts || !shorts.empty?
    # Honor tab=shorts only when data exists
    active_tab = (raw_tab && raw_tab.downcase == "shorts" && !shorts.empty?) ? "shorts" : "all"
    ...
    nav_params = [] of String
    nav_params << "max_results=#{max_results}" if env.params.query.has_key?("max_results")
    nav_params << "page=#{page}" if env.params.query.has_key?("page") && page > 1

    base_query = nav_params.join("&")
    all_tab_url = "/feed/subscriptions"
    all_tab_url += "?#{base_query}" unless base_query.empty?

    shorts_query = nav_params.dup
    shorts_query << "tab=shorts"
    shorts_tab_url = "/feed/subscriptions"
    shorts_tab_url += "?#{shorts_query.join("&")}" unless shorts_query.empty?
  ...
  def self.rss_private(env)
    ...
    videos, notifications, _shorts = get_subscription_feed(user, max_results, page)
