module Invidious::Channel::Tabs
  ...
  # Prefetch up to two pages of shorts (~60 items) for a smoother UX, mirroring videos/streams
  def get_60_shorts(channel : AboutChannel, *, continuation : String? = nil, sort_by = "newest")
    if continuation.nil?
      # Fetch the first "page" of shorts
      items, next_continuation = get_shorts(channel, sort_by: sort_by)
    else
      # Fetch a "page" of shorts using the given continuation token
      items, next_continuation = get_shorts(channel, continuation: continuation)
    end

    # If there is more to load, then load a second "page"
    # and replace the previous continuation token
    if !next_continuation.nil?
      items_2, next_continuation = get_shorts(channel, continuation: next_continuation)
      items.concat items_2
    end
