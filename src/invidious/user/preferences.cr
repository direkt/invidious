struct Preferences
  ...
  property shorts_max_length : Int32 = CONFIG.default_user_preferences.shorts_max_length
  ...
  module ShortsLengthConverter
    def self.to_json(value : Int32, json : JSON::Builder)
      json.number value
    end

    def self.from_json(value : JSON::PullParser) : Int32
      value.read_int.clamp(SHORTS_MIN_LENGTH, SHORTS_MAX_LENGTH).to_i32
    end
    ...
    def self.from_yaml(ctx : YAML::ParseContext, node : YAML::Nodes::Node) : Int32
      unless node.is_a?(YAML::Nodes::Scalar)
        return CONFIG.default_user_preferences.shorts_max_length
      end

      raw = node.value
      parsed = raw.empty? ? nil : raw.to_i?
      (parsed || CONFIG.default_user_preferences.shorts_max_length).clamp(SHORTS_MIN_LENGTH, SHORTS_MAX_LENGTH)
    end
  end
